...
Pro spuštění VISA sezení použijeme metodu správce zdrojů \lstinline|open_resource()|. Zde navíc specifikujeme ukončovací znaky pro čtení a zápis, což jsou znaky, které indikují, že přenos je ukončen (představte si vojenské filmy, kde říkají ,,přepínám'' do vysílačky). Běžné jsou nový řádek \ls{'\\n'}, návrat vozíku \ls{'\\r'} a jejich kombinace \ls{'\\r\\n'}. Naše Pico očekává, že příkazy budou ukončeny novým řádkem \ls{'\\n'} a své odpovědi také ukončuje novým řádkem. U jiných přístrojů je to něco, co musíte najít v manuálu, ale \ls{'\\n'} je nejběžnější.
...
Ve většině případů není pro úspěšné otevření sezení metodou \ls{rm.open_resource()} potřeba nic jiného než adresa zdroje. Nicméně, zejména u některých starších přístrojů připojených přes RS-232, jsou často vyžadovány další informace. Za předpokladu, že zařízení bylo otevřeno jako \ls{dev = rm.open_resource(address)}, je možné, že před zahájením komunikace musíme nastavit následující:
\begin{description}
...
\begin{exercise}
    Vytvořte program, který automaticky najde Pico, pokud je připojeno k počítači. Pošlete dotaz \ls{*IDN?} každému dostupnému VISA zdroji a najděte ten, který odpoví \ls{'PICO'}. Zdroje (sériové porty), které nejsou připojeny k žádnému přístroji, vyvolají timeout výjimku (timeout), která musí být ošetřena bez pádu programu.
\end{exercise}
...
\begin{syntax}[Animované grafy]
    interaktivní grafy je monžé animovat. Najjednoduchší spůsob je volat \ls{.set_data()} objektú, které vrací \ls{ax.plot()}. Minimální příklad je následující:
\begin{lstlisting}
...
for k in range(1, 1000):
    x = k/1000*2*np.pi
    y = np.sin(x)
    # rozšíříme seznam dat
    xs.append(x)
    ys.append(y)
    # a nastavíme nové hodnoty
    line[0].set_data(xs, ys)
    ax.relim() # přepočítá limity os
    ax.autoscale_view() # a automaticky změní měřítko os
    plt.pause(0.01) # interaktivní graf sa updatuje tu
\end{lstlisting}

Druhá, trocha více flexibilní možnost je použít třídu \ls{FuncAnimation} z modulu \ls{matplotlib.animation}, která dělá v podstate to samé, jen je možné např. nastavovat interval animace nebo animaci uložit do souboru. Minimální příklad je následující:
\begin{lstlisting}
import numpy as np
import matplotlib.pyplot as plt
...
# FuncAnimation periodicky volá funkci update a aktualizuje graf fig
# update() by mala příjmat jeden parameter který představuje číslo framu
# frames může byť příjmo číslo a update bude volaná s 1 ... frames-1 nebo
# např. seznam, pole, obecně libovolný iterovatelný objekt. interval udává
# interval mezi framami v ms
ani = animation.FuncAnimation(fig, update, frames=1000, interval=1, repeat=False)
# ani.save('animation.webm', writer='ffmpeg') # ak máte nainstalovaný ffmpeg
# ani.save('animation.gif', writer='pillow')
plt.show()
\end{lstlisting}

Jesli by sme chceli nekonečně běžící animaci můžeme použít iterátor \ls{count()} z modulu \ls{itertools} následujícím spůsobem

\begin{lstlisting}
...
from itertools import count
ani = animation.FuncAnimation(fig, update, frames=count(), interval=1, repeat=False)
plt.show()
\end{lstlisting}
\end{syntax}

\begin{exercise}
    Napište program, který bude kontinuálně měřit tlak a ukládat ho do textového souboru spolu s časem měření. Na grafu zobrazujte data vyhlazená s 1 s časovou konstantou.
\end{exercise}
