...
Kernel pro Eulerovské krokování je jednoduchý:
\lstinputlisting[linerange={48-59}]{../example_code/taichi/gravity.py}
To, že funkce je kernel, označíme pomocí dekorátoru \ls{ti.kernel}. Funkce přebírá čtyři argumenty -- polohy planet \ls{rs}, rychlosti \ls{vel} a zrychlení \ls{acc}, což jsou numpy pole 64bitových floatů tvaru \ls{(N,2)} ($N$ planet a dvě souřadnice) a časový krok \ls{dt}, což je jednoduchý float.

Uvnitř funkce vypadá poměrně obyčejně, nicméně běží uvnitř GPU a vnější cyklus for ve skutečnosti není cyklus, ale iterace se provádějí co nejvíce paralelně. Existují určitá omezení -- například se nedá použít \ls{break} ve vnějším cyklu. Vnitřní cyklus (\ls{for k in range(2)}) běží jako běžný sekvenční cyklus for. Nic nevracíme, ale spíše upravujeme vstupní pole.
