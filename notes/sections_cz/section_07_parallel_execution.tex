...
\begin{lstlisting}[linewidth=0.6\linewidth]
    # vlákno 2
    # běží vlákno 1
    lock.acquire() # blokuje do uvolnění zámku
    # běží vlákno 1
    # běží vlákno 1
...
\begin{exercise}
    Přepište program, který indikuje teplotu LEDkami tak, aby měření tlaku a aktualizace LEDek běželo v samostatných vláknech. Pro komunikaci mezi vlákny použijte frontu.
\end{exercise}

\begin{exercise}
    V separátních vláknech měřte teplotu a tlak, ukládejte do dvou separátních souborů. Zajistěte, že při ukončení programu pomocí Ctrl-C se oba soubory řádně zavřou.
\end{exercise}
...
\begin{exercise}
    Zrýchlite výpočet Mandelbrotovej množiny za pomoci paralelných procesov.
\end{exercise}
